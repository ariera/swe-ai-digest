# Plan v03: SQLite + CLI Split (feed / digest subcommands)
...
- `digests WHERE period covers target AND emailed_at IS NOT NULL` → already sent (default guard)
...
Before building a digest, the `digest` command checks the fetch failure rate:
- Count sources with `last_error IS NOT NULL`
- If failure rate exceeds threshold (e.g. >50% of enabled sources): send a health alert email to admin (`admin_address` from config) and log a prominent warning
- Does not block the digest — but the admin is notified and the warning is visible in logs
...
| Flag | Effect |
|------|--------|
| `--dry-run` | **Read-only.** Fetches and reports but makes no changes — no DB writes, no AI calls, no email, no feed/page updates. |
| `--no-email` | Full pipeline run (DB, AI, feed, pages) but skips only the email send. |
| `--admin-only` | Full pipeline run, email sent only to admin address. |
| `--force` | (`digest` only) Skip the "already sent" guard. Re-creates and re-sends a digest for a period that was already delivered. |
...
3. **Already-sent guard** (default behavior, no flag needed): check if a digest exists covering this period with `emailed_at IS NOT NULL`. If found → log and skip. Use `--force` to override.
...
5. Check source health: if failure rate is high, email admin alert
...
0 * * * * /path/to/.venv/bin/python3 /path/to/main.py feed >> /path/to/logs/cron.log 2>&1

# Digest: hourly on Mondays — command self-guards against re-sending
0 * * * 1 /path/to/.venv/bin/python3 /path/to/main.py digest >> /path/to/logs/cron.log 2>&1
```

The `digest` cron runs hourly on Mondays. The first successful run sends the email and marks `emailed_at`. All subsequent runs that day skip automatically. If the 7am run fails (API down), the 8am run retries. No `--scheduled` flag needed.
...
Keep existing `call_anthropic()` temporarily until Phase 3 is complete, then remove.
...
### Digest
- `Digest.for_period(session, period_start, period_end) → Digest | None` — find existing sent digest
...
### Standalone
- `sync_sources_from_yaml(session, sources_config)` — upserts all authors + sources from YAML. Module-level function in `db/models.py` since it orchestrates across Author + Source.
...
- [ ] Add and populate `slug` field to each engineer in `data/digest_sources.yaml`
...
- [ ] Implement `cmd_digest(args)`:
  - already-sent guard → feed pass (try/except) → check health → gather articles → global summary → pages → email → push
- [ ] Remove old code: `_should_run()`, `_already_succeeded_today/this_week()`, `_write_success_marker()`, `MARKER_FILE`, `build_digest()`, `save_digest()`, `_mock_ai_result()`, `_previous_iso_week_bounds()`
...
| `db/models.py` | **NEW** | All models with class/instance methods, sync_sources_from_yaml |
...
2. **DB smoke test:** creates DB with 5 tables
...
6. **Digest dry-run:** `python main.py digest --dry-run --debug` — reports what digest would contain
7. **Digest full run:** `python main.py digest --debug --admin-only` — full digest, email to admin only
8. **Already-sent guard:** run `digest` twice — second run skips ("already sent for this period")
9. **Force re-send:** `python main.py digest --force --admin-only` — re-creates and re-sends
10. **Idempotency:** run `feed` twice — second run finds no new articles
11. **Crash recovery:** kill `feed` mid-AI — next run picks up articles with `summary IS NULL`
12. **Health alert:** break a source, run feed + digest, verify admin gets health email
13. **Inspect DB:** `sqlite3 data/swe_ai_digest.db ".tables"` then `"SELECT count(*) FROM articles WHERE ai_relevant = TRUE"`
